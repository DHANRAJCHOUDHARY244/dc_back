...
class QuoteChatController {
    ...
    async sendChatToken(req: Request, res: Response) {
        try {
            ...
            if (!quote_id && !bypass_token) return ReE(res, SERVER_ERROR_CODE, "Quote ID and Bypass Token is required");
            const quote: any = await quoteRepository.findOne(
                { id: quote_id, bypass_token },
                {
                    populate: {
                        path: "customer",
            ...
            const token = jwt.sign(
                  ...
                  {expiresIn:"1d"});
            ReS(res,SUCCESS_CODE,"Token Genrated Successfully",{token,user:userData})
        } catch (error) {
            return ReE(res, SERVER_ERROR_CODE, `Server Error: ${error}`);
        }
    }
