...
import { accountsStockInvoiceRepository } from "@repositories";
...
class AccountsStockInvoiceController {
		...
		try {
			...
			if (search) {
				filter.$or = [
					...
					{ supplier_company: { $regex: search, $options: "i" } },
			...
			const { rows, count } = await accountsStockInvoiceRepository.findPaginated(filter, {
				page,
				limit,
			...
			const summaryRows = await accountsStockInvoiceRepository.aggregateRaw([
				{ $match: { deleted_at: null, ...(filter.status ? { status: filter.status } : {}), ...(filter.invoice_date ? { invoice_date: filter.invoice_date } : {}) } },
				{
					$group: {
						_id: "$status",
						total: { $sum: "$amount" },
			...
			return ReS(res, SUCCESS_CODE, "Accounts stock invoices fetched.", {
				currentPage: page,
			...
			console.error("[accountsStockInvoice.list]", err);
	...
	async getById(req: AuthenticatedRequest, res: Response) {
		try {
			...
			const doc = await accountsStockInvoiceRepository.findById(id, { lean: true });
			if (!doc) return ReE(res, BAD_REQUEST_CODE, "Stock invoice not found");
			return ReS(res, SUCCESS_CODE, "Stock invoice fetched.", doc);
		} catch (err: any) {
			return ReE(res, SERVER_ERROR_CODE, err.message || err);
		}
	}
	...
	async create(req: AuthenticatedRequest, res: Response) {
		try {
			...
			if (!data.invoice_number || !data.supplier_name || !data.invoice_date) {
				return ReE(res, BAD_REQUEST_CODE, "invoice_number, supplier_name and invoice_date are required");
			}
			const created = await accountsStockInvoiceRepository.create({
				...data,
				attachments: [],
				created_by: req.user?.id,
				updated_by: req.user?.id,
			});
			return ReS(res, SUCCESS_CODE, "Stock invoice created.", created);
		} catch (err: any) {
			return ReE(res, SERVER_ERROR_CODE, err.message || err);
		}
	}
	...
	async update(req: AuthenticatedRequest, res: Response) {
		try {
			const id = Number(req.params.id);
			if (!id) return ReE(res, BAD_REQUEST_CODE, "Invalid id");
			const existing = await accountsStockInvoiceRepository.findById(id);
			if (!existing) return ReE(res, BAD_REQUEST_CODE, "Stock invoice not found");

			const data = sanitizeBody(req.body || {});
			const updated = await accountsStockInvoiceRepository.updateById(id, {
				$set: { ...data, updated_by: req.user?.id },
			});
			return ReS(res, SUCCESS_CODE, "Stock invoice updated.", updated);
		} catch (err: any) {
			return ReE(res, SERVER_ERROR_CODE, err.message || err);
		}
	}
	...
	async remove(req: AuthenticatedRequest, res: Response) {
		try {
			const id = Number(req.params.id);
			if (!id) return ReE(res, BAD_REQUEST_CODE, "Invalid id");
			const existing: any = await accountsStockInvoiceRepository.findById(id, { lean: true });
			if (!existing) return ReE(res, BAD_REQUEST_CODE, "Stock invoice not found");
			...
			await accountsStockInvoiceRepository.deleteById(id);
			return ReS(res, SUCCESS_CODE, "Stock invoice deleted.");
		} catch (err: any) {
			return ReE(res, SERVER_ERROR_CODE, err.message || err);
		}
	}
	...
	async uploadAttachments(req: AuthenticatedRequest, res: Response) {
		try {
			...
			console.error("[accountsStockInvoice.upload]", err);
			return ReE(res, SERVER_ERROR_CODE, err.message || err);
		}
	}
...
export default new AccountsStockInvoiceController();
